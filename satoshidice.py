...
safe_recommendations = filter(lambda x: likelyness_dict[x] > 88, thresholds)
print "Safe bets (97% likely to succeed or better):"
for i in safe_recommendations:
     print "    ", i, likelyness_dict[i], "% likely to succeed"
    
if likelyness_dict[64000] < 97:
    print "Recommendation : wait"
else:
    global do
    do = "Recommendation : lessthan", safe_recommendations[-((len(safe_recommendations) / 2) - 1) if len(safe_recommendations) > 1 else -1]
...
plt.xlabel(do)
plt.title('last 90 bets evaluated')
